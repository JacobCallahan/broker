            ...
            for attr, obj in attrs.items():
                if attr == "provider_help":
                    ...
                    for _name, param in inspect.signature(obj).parameters.items():
                        if _name not in ("self", "kwargs", "settings_obj"):
                            # {_name: (cls, is_flag)}
                            PROVIDER_HELP[_name] = (
                                new_cls,
...
class Provider(metaclass=ProviderMeta):
    ...
    def __init__(self, settings_obj=None, **kwargs):
        ...
        if settings_obj:
            ...
            logger.debug(
                f"{cls_name} provider using custom settings object: {settings_obj.to_dict()}"
            )
        else:
    ...
    @abstractmethod
    def provider_help(self, settings_obj=None):
        """Help options that will be added to the CLI.

        Anything other than 'self', 'kwargs', and 'settings_obj' will be added as a help option
