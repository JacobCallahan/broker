...
def test_default_settings(broker_settings):
    test_provider = TestProvider(settings_obj=broker_settings)
    assert test_provider.instance == "test1"
    assert test_provider.foo == "bar"


def test_alternate_settings(broker_settings):
    test_provider = TestProvider(TestProvider="test2", settings_obj=broker_settings)
...
def test_validator_trigger(broker_settings):
    with pytest.raises(ConfigurationError) as err:
        TestProvider(TestProvider="bad", settings_obj=broker_settings)
...
@pytest.mark.parametrize("set_envars", [[("BROKER_TESTPROVIDER__foo", "override me")]], indirect=True)
