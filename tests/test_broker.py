...
def test_empty_init(broker_settings):
    ...
    broker_inst = Broker(settings_obj=broker_settings)
...
def test_kwarg_assignment(broker_settings):
    ...
    broker_inst = Broker(settings_obj=broker_settings, **broker_kwargs)
...
def test_full_init(broker_settings):
    ...
    broker_inst = Broker(hosts=broker_hosts, test_action="blank", nick="test_nick", settings_obj=broker_settings)
...
def test_specified_instance(broker_settings):
    ...
    broker_inst = Broker(nick="test_nick", TestProvider="test2", settings_obj=broker_settings)
...
def test_broker_e2e(broker_settings):
    ...
    broker_inst = Broker(nick="test_nick", settings_obj=broker_settings)
    host_checkout = broker_inst.checkout()
...
def test_broker_checkin_n_sync_empty_hostname(broker_settings):
    ...
    broker_inst = broker.Broker(nick="test_nick", settings_obj=broker_settings)
    ...
    broker_inst = broker.Broker(hosts=hosts, settings_obj=broker_settings)
    broker_inst.checkin()
...
def test_mp_checkout(broker_settings):
    ...
    # arround 0.5
    broker_inst = Broker(nick="test_nick", settings_obj=broker_settings, _count=VM_COUNT)
    broker_inst.checkout()
...
def test_mp_checkout_twice(broker_settings):
    broker_inst = Broker(nick="test_nick", settings_obj=broker_settings, _count=2)
