...
#      gh aw compile .github/agentic-workflows/wiki-sync.md
# ============================================================
        ...
        - name: Apply wiki changes
          ...
          run: |
            ...
            if [ -z "${WIKI_SYNC_PAT:-}" ]; then
              echo "::error::WIKI_SYNC_PAT secret is not set. See .github/agentic-workflows/wiki-sync.md for setup instructions."
              exit 1
            fi

            git clone \
              "https://x-access-token:${WIKI_SYNC_PAT}@github.com/SatelliteQE/broker.wiki.git" \
              wiki
            cd wiki
